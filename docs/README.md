...
- Cleanup audit (plan-043 pass, all findings applied): [smells](cleanup/smells.md) (severity-ranked code smells + what was done) · [standards](cleanup/standards.md) (STANDARDS/CONVENTIONS drift in `src/`, each resolved) · [perf](cleanup/perf.md) (`GameScene.update()` hot-path allocation audit) · [extensibility-seams](cleanup/extensibility-seams.md) ("edit data not code" registry seams to add) · [testability-targets](cleanup/testability-targets.md) (pure modules the seams create + tests they warrant)
- UI/HUD overhaul (plan 046, shipped as the `src/hud/` Field Kit): [ui-overhaul/README.md](ui-overhaul/README.md) — research, UX flows, the three design directions + which shipped; the DOM-vs-Phaser engine rationale in [build-stack-decision.md](ui-overhaul/build-stack-decision.md); interactive mockup of the candidates in [build-ui-options.html](build-ui-options.html)
