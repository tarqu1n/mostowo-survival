...
- **Tiling palette** — named, **global (cross-map)** palettes in their own file `src/data/maps/palettes.json` (mirrors `nodes.json`; auto-committed via editor autocommit, so they sync across devices). **Auto-saved**: adding/removing a tile or creating a palette persists immediately (debounced) — **no map Save press needed** — and does NOT touch the open map's dirty state or undo stack (global curation isn't map history). **Fill** a palette in the Library: toggle the **palette+plus** button (desktop: in the Library panel; compact: in the Library drawer's bottom bar), multi-tap tiles, then **"Add (N)"**. **Tap a slot** to arm that tile on the brush; **long-press a slot to remove** it. Switch palettes via the dropdown; **"＋"** creates a new one. Slots wrap to up to 3 rows. Single tile per slot (no NxM stamps). The active-palette pointer is editor view-state. **Rename a palette by editing `palettes.json`** — not in the UI yet.
