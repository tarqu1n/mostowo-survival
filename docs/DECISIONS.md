...
Reworking the **build experience** into a legible **Blueprint Mode** is the chosen next focus (directions explored in [build-ui-options.html](build-ui-options.html), critique #2/#3). Settled calls: placement resolves on **pointer-UP** (build-mode drag pans; command-mode queue-paint gated off in build mode); the line tool paints an **axis-locked pending run** (pure `systems/buildRun.ts`) that costs **nothing** until a commit bar's **Confirm** spends + queues only the **affordable-and-placeable subset** (Cancel drops it); per-buildable **build time** (`systems/buildTime.ts`); a HUD **rotation ring** + `R`/`Shift+R` with a backward-compatible extended `build:rotate {dir?,to?}`; Blueprint-Mode dim + snap grid; a per-site construction **scaffold** carrying the build progress bar. **Buildable-icon ART is deferred** — the `BuildableIcon` render path + `BuildableDef.icon` field ship, but no icon files (colour-swatch fallback everywhere).

→ [architecture.md](decisions/architecture.md#2026-07-24--decided-build-ux-is-the-chosen-focus-blueprint-mode-overhaul-plan-050-pointer-up-placement-line-tool-pending-run--commit-bar-per-buildable-build-time-rotation-ring)
...
The first equippable items. **Three equip slots** (`mainHand`/`ranged`/`offHand`) with an **empty default loadout** — unarmed melee (= today), no ranged until a bow is equipped, empty off hand. A new **`Equipment` pure system** mirrors `Inventory`; **durability lives only on `Equipment`, never on the bag `Slot`**. The **brand is equip-to-consume** (equip spends 1 + seeds durability; unequip/destroy discards, no partial restash); **permanent gear (bow/sword) moves bag↔slot freely**. Combat: main-hand item → active `MELEE_WEAPONS` entry (empty → unarmed); **ranged is gated on an equipped bow** (`combat:bow` no-ops otherwise, Bow button hidden) — the crafted bow is the first ranged weapon, a deliberate change from the old always-on bow. Brand light is implemented by **`playerLight()` raising its radius** (the config-prescribed "a future off-hand torch just raises this radius") into SurvivalClock's existing night-overlay union — no parallel disc; fog unchanged. Out of scope: body rendering (→ plan 010), armour/shield slots, ammo, NPC equipment.

→ [gameplay.md](decisions/gameplay.md#2026-07-24--decided-equippable-items--equip-slots-plan-049-3-slots-empty-default-loadout-durability-on-equipment-only-brand-via-playerlight-bow-not-default-gate)
