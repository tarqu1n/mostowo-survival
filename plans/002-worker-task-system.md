...
- [ ] **Step 1: A* pathfinding module** `[delegate]` (parallel: A)
  ...
  - **8-connectivity**, disallowing diagonal moves that cut between two blocked orthogonal corners
    (no squeezing through wall diagonals). Octile distance heuristic. Treat `goal` as reachable even
    if the tile itself is technically the target; if `start === goal` return `[]`. If `goal` is blocked
    or unreachable, return `null` (the caller handles "path to an adjacent tile" for harvest/build).
    Never include `start` in the returned path (return the sequence of tiles to step *to*).
  - Also export `neighbors`/`isDiagonalBlocked` only if helpful internally — keep the public surface to
    `findPath`. Add a tiny `manhattan`/`octile` helper as needed. No magic numbers — costs as consts
  ...
  - Done when: `npm run typecheck` passes; a hand-traced case works — on a 5×5 open grid,
    ...
    (length > 2) rather than returning null.
...
- [ ] **Step 4: Task queue + tap/long-press input** `[inline]`
  - Introduce a `TaskQueue` (Step 2) in `GameScene`. Refactor the pointer gate: resolve move/harvest
    orders on **`pointerup`** using `pointer.getDuration()` — `< LONGPRESS_MS` → `queue.replace(action)`;
    `>= LONGPRESS_MS` → `queue.append(action)`. Keep the **UI-guard** (`hudHitTest`) and the build-mode
    seam. Ignore large drags (movement between down/up over a few px) so a stray swipe isn't an order.
  ...
  - **Executor:** a single `runCurrent(delta)` in `update` that switches on `current.kind`:
    `move` → follow path, done when arrived; `harvest` → path to an adjacent walkable tile of the tree
    (nearest of its 8 neighbours that isn't blocked), then run the chop loop until felled, then done;
    when an action finishes, `queue.next()` and recompute its path. Clear `current` when the queue
    drains. A `harvest` whose tree is already a stump completes immediately.
...
- [ ] **Step 5: Timed construction — blueprints + build task** `[inline]`
  - Model a construction **site**: `{ id, col, row, rect, progress:number, done:boolean }`. In build
    ...
    it to `occupied`/`isBlocked` yet (blueprints are passable). Ghost validity now also rejects tiles
    that already hold a site.
  - **Build executor** (`current.kind === 'build'`): path to an adjacent walkable tile of the site,
    then accumulate `progress += delta` until `>= BUILD_MS` (config). Reflect progress visually (ramp
    the blueprint's alpha / fill toward the finished wall colour). On completion: mark `done`, recolour
    to `BUILDABLES.wall.color` at full alpha, create the **static wall body** (as in 001), add the tile
    to `occupied` so `isBlocked` now returns true, and **re-path** the worker if needed. Finish the task.
...
- [ ] **Step 7: Verify end-to-end + docs** `[inline]`
  - **Extend `scripts/smoke.mjs`:** (a) place a wall directly between the player and a target, order a
    move past it, and assert the player ends adjacent to the goal (path detoured, not stuck against the
    wall); (b) long-press two tiles and assert the queue indicator/`pending` reaches 2 and drains; (c)
    place a blueprint, assert wood drops by 2 immediately, that the tile is **not** blocking while
    building, and that after ~`BUILD_MS` it becomes blocking (path now routes around it); (d) Cancel
    clears the queue but a placed blueprint remains. Keep the existing chop/build/no-console-error
    assertions. Expose whatever `GameScene` state the asserts need (e.g. `path.length`, `sites.length`,
    `queue.pending`) on the instance.
