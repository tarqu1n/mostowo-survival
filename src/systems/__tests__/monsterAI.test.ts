...
/** Deterministic PRNG (mulberry32) so seeded runs are reproducible — thin wrapper over the shared
 *  `systems/rng.ts` factory, kept so the many `mulberry32(seed)` call sites below don't need touching. */
function mulberry32(seed: number): () => number {
  return makeRng(seed).nextFloat;
}
